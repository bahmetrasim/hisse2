...
st.set_page_config(page_title="Anayasa v14", layout="centered")
st.title("Yatırım Anayasası v14")

if st.button("50'şerli Paketler Halinde Taramayı Başlat"):
    uygunlar = []
    
    # 50'şerli paket döngüsü (Hatasız)
    for i in range(0, len(FULL_LIST), 50):
        paket = FULL_LIST[i:i+50]
        st.write(f"📦 Paket {i//50 + 1} taranıyor... ({len(paket)} hisse)")
        ...
        st.write(f"✅ Paket {i//50 + 1} tamamlandı. Güvenlik molası...")
        time.sleep(10)
